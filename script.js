...
function init() {
    for (let index = 0; index < bundeslaender.length; index++) {
        const element = bundeslaender[index];
        document.getElementById('bundes-land-reihen').innerHTML += /*html*/`
        <div id="${index}"class="bundesLand" onmouseover="showURL(id)" onmouseout="hideURL(id)">
            <span><b>${element['name']}</b></span>
            <span>${element['population']}</span>
            <span id="bundesland-${index}" class="bundeslandURL d-none">${bundeslaender[index]['url']}</span>
        </div>
...
function showURL(given_id) {
    let showMe = `bundesland-${given_id}`;
    document.getElementById(showMe).classList.remove('d-none');
}


function hideURL(given_id) {
    let hideMe = `bundesland-${given_id}`; 
